    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:5000/api/connections/users",
          {
            method: "GET",
            headers: {
  ...
  const startChat = async (receiverId) => {
    ...
    try {
      ...
      if (chatResponse.ok) {
        ...
        navigate(
          `/user/${localStorage.getItem("userId")}/chat?chatId=${chatData._id}`
        );
      } else {
        ...
        <ul>
          {users.map((user) => (
            <li key={user._id}>
              ...
              <div className="ConnectPage-btns">
                <button onClick={() => sendConnectionRequest(user._id)}>
                  Connect
                </button>
                <button onClick={() => startChat(user._id)}>Message</button>{" "}
                {/* Update this line */}
              </div>
            </li>
          ))}
